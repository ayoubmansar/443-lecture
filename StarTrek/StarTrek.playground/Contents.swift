...
var femaleHumans = humanOfficers.filter { femaleOfficers.contains($0) }
var percent = Float(femaleHumans.count) / Float(humanCount) * 100

// Q2: How many first officers are human?
var humanFirstOfficers = firstOfficers.filter { humanOfficers.contains($0.key) }
var numberHumanFirstOfficers = humanFirstOfficers.count

// Q3: Get a list of females sorted by name
var sortedFemales = femaleOfficers.sorted(by: <)
/*:
...
let a = captains.map{ Officer(name: $0, ship: $1)}.sorted(by: { $0 < $1})
print(a.map{$0.name})
/*:
Group 2
*/
// Q6: Create an array of officers serving as first officers who are also Vulcan
// create an arr of first officer objects
// filter thru that list and find the first officers that are vulcan (only 2 — spock and ?)
let allOfficers: [Officer] = firstOfficers.map { Officer(name: $0, ship: $1)}
let vulcanOfficersFiltered = allOfficers.filter { vulcanOfficers.contains($0.name) }
print(vulcanOfficersFiltered.map{$0.name}) // Spock and Tpol

// Q7: How many these Vulcan first officers have names shorter than 5 characters?
let lessThanFive = vulcanOfficersFiltered.reduce(0, { total, curr in total + (curr.name.count < 5 ? 1 : 0) })
print(lessThanFive) // Only 1 officer
